...
def build_json(
    ...
    # Add MCP Details section if present (Reports v2)
    if mcp_details:
        # Apply PII masking to MCP details if requested
        if anonymize:
            mcp_details = _mask_mcp_details(mcp_details)
        report["mcp_details"] = mcp_details
...
def _mask_mcp_details(mcp_details: Dict[str, Any]) -> Dict[str, Any]:
    """Apply PII masking to MCP details."""
    masked_details = mcp_details.copy()
    
    if "sessions" in masked_details:
        masked_sessions = []
        for session in masked_details["sessions"]:
            masked_session = session.copy()
            
            if "steps" in masked_session:
                masked_steps = []
                for step in masked_session["steps"]:
                    masked_step = step.copy()
                    
                    # Mask text fields that might contain PII
                    for field in ["input_text", "output_text"]:
                        if field in masked_step and masked_step[field]:
                            masked_step[field] = mask_text(str(masked_step[field]))
                    
                    # Mask tool arguments if present
                    if "tool_args" in masked_step and masked_step["tool_args"]:
                        masked_args = {}
                        for key, value in masked_step["tool_args"].items():
                            if isinstance(value, str):
                                masked_args[key] = mask_text(value)
                            else:
                                masked_args[key] = value
                        masked_step["tool_args"] = masked_args
                    
                    masked_steps.append(masked_step)
                
                masked_session["steps"] = masked_steps
            
            masked_sessions.append(masked_session)
        
        masked_details["sessions"] = masked_sessions
