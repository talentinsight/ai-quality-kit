...
def write_excel(path: str, data: Dict[str, Any]) -> None:
    ...
    # Optional sheet for MCP details (Reports v2)
    if data.get("mcp_details"):
        _create_mcp_details_sheet(wb, data)
...
def _create_mcp_details_sheet(wb: Workbook, data: Dict[str, Any]) -> None:
    """Create MCP Details sheet with step-by-step traces."""
    ws = wb.create_sheet("MCP_Details")
    
    # Headers
    headers = [
        "Session ID", "Step ID", "Role", "Channel", "Tool Name", "Decision", 
        "Latency (ms)", "Tokens In", "Tokens Out", "Cost ($)", 
        "Guardrail Violations", "Reused Fingerprints", "Error"
    ]
    ...
    # Data rows
    row_idx = 2
    mcp_details = data.get("mcp_details", {})
    
    for session_data in mcp_details.get("sessions", []):
        ...
        for step in session_data.get("steps", []):
            ...
            if len(input_text) > 50:
                input_text = input_text[:25] + "..." + input_text[-20:]
            
            output_text = step.get("output_text", "")
            if len(output_text) > 50:
                output_text = output_text[:25] + "..." + output_text[-20:]
            
            # Format guardrail violations
            violations = []
            for signal in step.get("guardrail_signals", []):
                if signal.get("label") in ["violation", "hit"]:
                    violations.append(f"{signal.get('id', 'unknown')}:{signal.get('score', 0):.3f}")
            violations_str = ", ".join(violations) if violations else "None"
            
            # Format reused fingerprints
            reused_fps = step.get("reused_fingerprints", [])
            reused_str = f"{len(reused_fps)} reused" if reused_fps else "None"
